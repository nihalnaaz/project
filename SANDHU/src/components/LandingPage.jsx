        ...
        {!isLoginOpen ? (
          ...
          <div className="w-full max-w-sm p-8 bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl border border-white dark:border-slate-700 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] animate-in fade-in slide-in-from-bottom-8 duration-500">
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              {/* Demo Login Link */}
              {isPatient && !isRegisterMode && (
                <button
                  type="button"
                  onClick={() => onLogin('patient', 'demo@patient.com', 'demo', '+1 0000000000', true)}
                  className="w-full mt-2 py-3 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-gray-700 dark:text-slate-200 font-semibold rounded-xl transition-all shadow-sm flex items-center justify-center gap-2"
                >
                  <Activity className="w-4 h-4" /> Quick Demo Login
                </button>
              )}
